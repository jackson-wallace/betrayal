...
export class Game {
  ...
  constructor(canvas: HTMLCanvasElement, ws: WSDriver) {
    this.currentPlayer = new Player("1", "LightSkyBlue", { r: 9, q: 9 });
    this.state = new GameState("1");
    this.state.addPlayer(this.currentPlayer); // Add the player to the state
    this.ws = ws;
    this.display = new DisplayDriver(canvas, this.state);
    this.initGame();
  }
...
export class GameState {
  private _players: Record<string, Player>; // Map of playerID to Player
  private _currentPlayerId: string;
  private _status: "waiting" | "active" | "completed";
  ...
  addPlayer(player: Player): void {
    this._players[player.id] = player; // Add player by ID
  }

  removePlayer(playerId: string): void {
    delete this._players[playerId]; // Remove player by ID
  }
